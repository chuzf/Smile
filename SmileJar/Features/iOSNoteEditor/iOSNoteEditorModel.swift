...
/// Draft attachment for the iOS note editor
public struct DraftAttachment: Identifiable, Equatable {
    /// Unique identifier per session
    public let id: UUID

    /// Persisted identifier after saving (nil until saved)
    public var persistedID: PersistentIdentifier?

    /// Media kind (audio, video, image, etc.)
    ...
    public init(
        ...
        persistedID: PersistentIdentifier? = nil
    ) {
        self.id = UUID()
        self.kind = kind
